...
const Activecustomer = () => {
  ...
  useEffect(() => {
    axios
      ...
      .then((res) => {
        ...
        const setupPendingCust = responseData.setup_pending_cust || [];
  ...
  return (
    ...
    <h4 className='tophead'>Pending <i class="fa-solid fa-clock"></i></h4>
            ...
            <tbody>
              {filteredActive.map((item) => (
                <tr key={item.company_id}>
                  ...
                  <td className='tdclassone'><Link to={`/${item.company_id}`} key={item.company_id}><i class="fa-solid fa-address-card"></i></Link></td>
                </tr>
              ))}
            </tbody>
          </table>
